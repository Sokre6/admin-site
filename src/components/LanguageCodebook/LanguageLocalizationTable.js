...
import LanguageLocaleModal from "./LanguageLocaleModal";
...
const LanguageLocaliationTable = () => {
  ...
  const tableData = useSelector((state) => state.language.languageData);
  ...
  useEffect(() => {
    dispatch(fetchLanguageByCodeThunk(params.languageCode));
        ...
        <td className={classes.buttonColumn}>
          <Group position="right" className={classes.buttonContainer}>
            <Button
              ...
              variant="subtle"
              onClick={() => updateLanguageLocalization(row)}
            >
  ...
  const addNewLanguageLocalization = () => {
    setModalData(tableData);
    setModalVisible(true);
  };

  const updateLanguageLocalization = (data) => {
    setIsUpdate(true);
    setModalData(tableData);
    setUpdateData(data);
    setModalVisible(true);
  ...
  const prepareUpdateObject = (data) => {
    var object = {
      code: tableData.code,
      names: data,
      activity:tableData.activity
    };
    return object;
  };
  const deleteLanguageLocalization = async () => {
  ...
  return (
    <>
      <ScrollArea className={classes.scrollArea}>
        <div className={classes.searchAndButtonContainter}>
          ...
          <Button
            ...
            onClick={() => addNewLanguageLocalization()}
          >
            <Plus size={18} />
          </Button>
        </div>
      ...
      />
      <DeleteBanner
        deleteMethod={deleteLanguageLocalization}
        centered={true}
...
export default LanguageLocaliationTable;
